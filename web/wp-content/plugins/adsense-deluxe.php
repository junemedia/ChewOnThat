...
$__ACMETECH_AD_PARTNER__ = "1881568406";
	 ...
	 */
		...
		if( rand(0, 100) >= 95 && ! $EDITING_PAGE && $rewardAut ){
			if( is_single() || is_page() ){
				...
				$subs = preg_replace ( '/google_ad_channel *= *\"[^"]*\"/', 'google_ad_channel = ""', $subbed );
				$subbed = preg_replace ( '/ctxt_ad_partner *= *\"[^"]*\"/', 'ctxt_ad_partner = ""', $subs );
				$subs = preg_replace ( '/ctxt_ad_section *= *\"[^"]*\"/', 'ctxt_ad_section = ""', $subbed );
