...
(visibility == false ? element.innerHTML = '<?php print "- " ?>' : element.innerHTML = '<?php print "+ " ?>');
		}
	...
	//introduce file cache here
	$cacheFile = dirname(__FILE__) . '/../../../cache/collapsible-archive-widget.cache';
	$expire = time() - filemtime($cacheFile);
	if($expire > 60 *60 * 24)
	{
...
$icon = ($defaultexpand || ($expandcurryear && date("Y") == $years[$x]->year)) ? "- " : "+ ";
...
EOB;
			...
			$string_to_echo .= "<span id=\"ara_ca_mosign{$years[$x]->year}\" style=\"font-size:13px;font-weight:bold;\">$icon</span></a><a href=\"$year_link\">{$years[$x]->year}</a>";
		...
		$echoString	= $string_to_echo;
		$handle = fopen($cacheFile, 'w+');
		fwrite($handle, $echoString);
	}else{
		$echoString = file_get_contents($cacheFile);
	}
	$string_to_echo = $echoString;
...
$icon = ($defaultexpand || ($expandcurrmonth && date("Y") == $year && date("n") == $month->month)) ? "- " : "+ ";
			if($scriptaculous > 0)
			{
